...
def optimize_all(seed, pairfile, num_decoders, reruns, batch_size=500):
    ...
    for rerun in reruns:
        ...
        for decoder_idx in range(num_decoders):
            spline_path = f"src/artifacts/spline_ensemble_seed{seed}_p{pair_tag}_rerun{rerun}_dec{decoder_idx}.pt"
            output_path = f"src/artifacts/spline_ensemble_optimized_seed{seed}_p{pair_tag}_rerun{rerun}_dec{decoder_idx}.pt"
...
if __name__ == "__main__":
    ...
    optimize_all(args.encoder_seed, args.pairfile, args.num_decoders, args.reruns)
