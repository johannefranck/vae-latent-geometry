...
def main():
    ...
    for rerun in range(N_RERUNS):
        seed = 1000 + rerun # fix seed for encoder determinism
        set_seed(seed)

        model = EVAE(input_dim=input_dim, 
                     latent_dim=LATENT_DIM, 
                     num_decoders=1).to(DEVICE)
    ...
    for num_decoders in DECODER_COUNTS:
        ...
        for rerun in range(N_RERUNS):
            ...
            # === Set new seed for decoder initialization
            decoder_seed = rerun * 100 + num_decoders
            set_seed(decoder_seed)  # resets randomness for decoder weights only
